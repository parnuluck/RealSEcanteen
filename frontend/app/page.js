...
export default function Home() {
  ...
  const loadMyQueue = async () => {
    try {
      const res = await api.get("/myQueue");
  ...
  const leaveTable = async (tableId) => {
    try {
      const res = await api.post("/leave", { tableId });
      alert(res.data.message);
      loadTables();
      loadQueue();
    } catch (err) {
  ...
  const reserve = async () => {
    const hasGreen = tables.some(t => (6 - t.people) >= people);
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        const res = await api.post("/reserve", { people, lat, lng, tableId: selectedTable });
        alert(res.data.message);
        setShowPopup(false);
  ...
  const startScan = () => {
    try {
      ...
      scanner.render(async (decodedText) => {
        try {
          const res = await api.post("/scan", { tableId: decodedText });
          alert(res.data.message);
          loadTables();
          loadQueue();
          scanner.clear();
  ...
  const callQueue = async () => {
    const res = await api.post("/callQueue", {});
    alert(res.data.message);
    loadQueue();
  };
